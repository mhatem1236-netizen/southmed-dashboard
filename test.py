    ...
    if uploaded_file is not None:
        ...
        if 'Company Name' in filtered_df.columns and 'Sampling Location' in filtered_df.columns:
            ...
            if all_log_companies:
                ...
                # تم إضافة التابة الخاصة بالـ Quantities Rate
                ...
                # ==========================================
                # 🚀 القنبلة الموقوتة: قسم Quantities Rate الجديد بالكامل
                # ==========================================
                with tab_quantities:
                    st.markdown(f"### 📊 Execution Quantities Analytics")
                    st.caption("Comprehensive review of site execution, targets, and lab validation.")
                    
                    # 1. سحب أسماء العواميد بدقة حسب المطلوب
                    contractor_col = next((c for c in df.columns if 'CONTRACTOR' in c.upper()), None)
                    comp_main_col = next((c for c in df.columns if str(c).strip().upper() in ['COMPANY', 'COMPANY NAME']), None)
                    
                    exec_qty_m3_col = next((c for c in df.columns if 'EXECUTED QUANTITY' in c.upper() and 'M3' in c.upper()), None)
                    exec_qty_lab_col = next((c for c in df.columns if str(c).strip().upper() == 'EXECUTED QUANTITY'), None)
                    ...
                    elem_all_col = next((c for c in df.columns if 'ELEMENT (ALL)' in c.upper()), None)
                    elment_col = next((c for c in df.columns if 'ELMENT' == str(c).strip().upper() or ('ELEMENT' == str(c).strip().upper() and 'ALL' not in str(c).upper())), None)

                    if contractor_col and comp_main_col:
                        all_comps = pd.concat([df[contractor_col], df[comp_main_col]]).dropna().astype(str).str.strip()
                        valid_companies = sorted(list(set([c for c in all_comps if c.lower() != 'nan' and c != ''])))
                        
                        exec_comp = st.selectbox("🏗️ Select Contractor for Quantities Analysis:", valid_companies, key="q_comp_sel")
                        
                        df_site = df[df[contractor_col].astype(str).str.strip().str.lower() == exec_comp.lower()].copy()
                        df_lab = df[df[comp_main_col].astype(str).str.strip().str.lower() == exec_comp.lower()].copy()
                        
                        if not df_site.empty or not df_lab.empty:
                            
                            # --- Requirement 1, 2, 3: The Cards ---
                            st.markdown("#### ⚖️ Overall Quantities KPIs")
                            c1, c2, c3 = st.columns(3)
                            
                            # Card 1: Total Project Scope (From Company & Total Quantity)
                            total_scope = 0
                            if total_qty_col and not df_lab.empty:
                                df_lab[total_qty_col] = pd.to_numeric(df_lab[total_qty_col], errors='coerce').fillna(0)
                                if elment_col:
                                    total_scope = df_lab.groupby(elment_col)[total_qty_col].max().sum()
                                else:
                                    total_scope = df_lab[total_qty_col].max()
                            create_card(c1, "Total Project Scope (m³)", f"{total_scope:,.1f}", delta_html=f"<span style='color:#bdc3c7; font-size:11px;'>Ref: {comp_main_col} ➔ {total_qty_col}</span>")

                            # Card 2: Lab Executed (From Company & Executed Quantity)
                            lab_exec = 0
                            if exec_qty_lab_col and not df_lab.empty:
                                ...
                                lab_exec = df_lab[exec_qty_lab_col].sum()
                            create_card(c2, "Lab Executed Qty [QC]", f"{lab_exec:,.1f}", delta_html=f"<span style='color:#ffaa00; font-size:11px;'>Ref: {comp_main_col} ➔ {exec_qty_lab_col}</span>")

                            # Card 3: Site Executed (From Contractor & Executed Quantity m3)
                            site_exec = 0
                            if exec_qty_m3_col and not df_site.empty:
                                df_site[exec_qty_m3_col] = pd.to_numeric(df_site[exec_qty_m3_col], errors='coerce').fillna(0)
                            ...
                            # --- Requirement 7: Missing Elements Audit ---
                            st.markdown("#### 🕵️ Elements Audit (Technical Office vs Lab)")
                            if elem_all_col and elment_col:
                                site_elems = set(df_site[df_site[exec_qty_m3_col] > 0][elem_all_col].dropna().astype(str).str.strip()) if exec_qty_m3_col else set()
                                lab_elems = set(df_lab[elment_col].dropna().astype(str).str.strip())
                                
                                missing_in_lab = site_elems - lab_elems
                                if missing_in_lab:
                                    missing_str = " ، ".join(missing_in_lab)
                                    st.markdown(f"""
                                    <div style="background: rgba(231, 76, 60, 0.1); border-left: 5px solid #e74c3c; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
                                        <h4 style="color: #e74c3c; margin-top: 0; font-size: 16px;">🚨 URGENT: MISSING QC PAPERWORK FOR EXECUTED ELEMENTS</h4>
                                        <p style="color: {ui['text_main']}; margin: 5px 0 0 0; font-size: 14px;">
                                            The contractor executed quantities in the elements below (logged in Site), but NO laboratory tests exist for them in (Lab).
                                            <br><i>Please request the missing paperwork from the Technical Office immediately.</i>
                                        </p>
                                        <br><b style="color:white; font-size:16px;">{missing_str}</b>
                                    </div>
                                    """, unsafe_allow_html=True)
                                else:
                                    st.success("✅ All executed elements have corresponding Lab records.")
                            else:
                                st.info(f"Need '{elem_all_col}' and '{elment_col}' to perform audit.")
                                
                            st.markdown('<div class="gradient-divider"></div>', unsafe_allow_html=True)

                            # --- Requirement 4 & 5: Charts and Breakdown ---
                            col_chart, col_elem = st.columns([0.6, 0.4])
                            
                            with col_chart:
                                st.markdown("#### 🚀 Daily Target vs Execution")
                                if date_daily_col and target_col and exec_qty_m3_col and not df_site.empty:
                                    ...
                                    if not valid_dates_df.empty:
                                        valid_dates_df[target_col] = pd.to_numeric(valid_dates_df[target_col], errors='coerce').fillna(0)
                                        daily_trend = valid_dates_df.groupby(date_daily_col).agg({
                                            exec_qty_m3_col: 'sum',
                                            target_col: 'max'
                                        }).reset_index().sort_values(date_daily_col)
                                        
                                        days_hit_target = len(daily_trend[daily_trend[exec_qty_m3_col] >= daily_trend[target_col]])
                                        total_active_days = len(daily_trend)
                                        st.markdown(f"**Target Hit Rate:** <b style='color:#2ecc71;'>{days_hit_target} / {total_active_days}</b> days.", unsafe_allow_html=True)
                                        
                                        fig_daily = go.Figure()
                                        fig_daily.add_trace(go.Scatter(x=daily_trend[date_daily_col], y=daily_trend[target_col], name="Target Daily Rate", mode='lines', line=dict(color='#e74c3c', width=4, shape='linear')))
                                        fig_daily.add_trace(go.Bar(x=daily_trend[date_daily_col], y=daily_trend[exec_qty_m3_col], name="Executed (m³)", marker_color='#00d2ff', opacity=0.85))
                            ...
                            with col_elem:
                                st.markdown("#### 🏗️ Execution per Element (Site)")
                                if elem_all_col and exec_qty_m3_col and not df_site.empty:
                                    ...
                                    if not elem_df.empty:
                                        fig_el = px.bar(elem_df, x=exec_qty_m3_col, y=elem_all_col, orientation='h', text_auto=True, color_discrete_sequence=['#9b59b6'])
                                        fig_el.update_layout(height=350, margin=dict(l=0, r=0, t=10, b=0))
                                        try: fig_el = style_3d_glassy(fig_el, chart_type="bar")
                                        except: pass
                                        st.plotly_chart(fig_el, use_container_width=True)
                                    else:
                                        st.info("No >0 quantities found for elements.")
                                        
                            st.markdown('<div class="gradient-divider"></div>', unsafe_allow_html=True)
                            
                            # --- Requirement 8: Worst Contractor per Sector ---
                            st.markdown("#### 🏆 Sector Performance (Worst Contractor Detection)")
                            if sector_col and contractor_col and target_col and exec_qty_m3_col:
                                perf_df = df.copy()
                                perf_df[exec_qty_m3_col] = pd.to_numeric(perf_df[exec_qty_m3_col], errors='coerce').fillna(0)
                                perf_df[target_col] = pd.to_numeric(perf_df[target_col], errors='coerce').fillna(0)
                                
                                perf_df = perf_df[(perf_df[target_col] > 0) & (perf_df[contractor_col].notna())]
                                
                                if not perf_df.empty:
                                    sector_stats = perf_df.groupby([sector_col, contractor_col]).agg({
                                        exec_qty_m3_col: 'sum',
                                        target_col: 'sum' # Assuming target is per row, adjust if it's max per day
                                    }).reset_index()
                                    
                                    sector_stats['Completion %'] = (sector_stats[exec_qty_m3_col] / sector_stats[target_col] * 100).round(1)
                                    sector_stats = sector_stats[sector_stats[exec_qty_m3_col] > 0] # Only evaluate active contractors
                                    
                                    if not sector_stats.empty:
                                        sectors = sector_stats[sector_col].unique()
                                        s_cols = st.columns(len(sectors))
                                        
                                        for idx, s in enumerate(sectors):
                                            s_df = sector_stats[sector_stats[sector_col] == s]
                                            worst = s_df.loc[s_df['Completion %'].idxmin()]
                                            
                                            s_cols[idx].markdown(f"""
                                            <div style="background: rgba(231, 76, 60, 0.05); border: 1px solid rgba(231, 76, 60, 0.3); border-left: 4px solid #e74c3c; padding: 15px; border-radius: 8px;">
                                                <div style="color: {ui['text_muted']}; font-size: 12px; text-transform: uppercase; font-weight: bold;">Lowest Yield in {s}</div>
                                                <div style="color: #e74c3c; font-size: 22px; font-weight: 800; margin: 5px 0;">{worst[contractor_col]}</div>
                                                <div style="color: {ui['text_main']}; font-size: 14px;">Target Met: <b style="color:#e74c3c;">{worst['Completion %']}%</b></div>
                                            </div>
                                            """, unsafe_allow_html=True)
                                    else:
                                        st.info("Not enough active data to calculate sector performance.")
                            else:
                                st.info("Requires Sector, Contractor, Target, and Executed (m3) columns.")

                        else:
                            st.warning("No data found for this contractor.")
                    else:
                        st.warning("Missing 'Contractor' or 'Company' columns in the dataset.")
