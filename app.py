...
def render_dashboard():
    ...
    if uploaded_file is not None:
        ...
        # 💡 تم الإصلاح الآمن لمنع KeyError في حالة عدم وجود العمود
        ...
        if 'Company Name' in filtered_df.columns and 'Sampling Location' in filtered_df.columns:
            ...
            if 'Company Name' in df.columns and 'Required Quantity' in df.columns:
                ...
                for _, row in lookup_df.iterrows():
                    c_key = str(row['Company Name']).strip().lower() 
            ...
            # 💡 التعديل الجذري والآمن اللي بيمنع KeyError للأبد
            log_companies = [str(c).strip() for c in mat_df['Company Name'].dropna().unique()] if 'Company Name' in mat_df.columns else []
            
            # إذا كان العمود اسمه Company خده، وإلا دور على Company Name 
            comp_temp_name = 'Company' if 'Company' in df.columns else ('Company Name' if 'Company Name' in df.columns else None)
            target_companies = [str(c).strip() for c in df[comp_temp_name].dropna().unique()] if comp_temp_name else []
                    ...
                    if comp_name2_col and exec_qty_col:
                        ...
                        if not comp_qty_df.empty:
                            ...
                            if date_daily_col:
                                ...
                                valid_dates_df = valid_dates_df[valid_dates_df[exec_qty_col] > 0] 
