...
# Theme & Presentation Initialization
...
def render_dashboard():
    ...
    # --- SIDEBAR CONTROLS ---
    st.sidebar.markdown("### 🎨 Presentation & UI")
    ...
    st.sidebar.markdown("### 📺 Auto-Play Kiosk")
    auto_play_toggle = st.sidebar.toggle("▶️ Start Auto-Presentation", value=st.session_state["auto_play"])
    if auto_play_toggle != st.session_state["auto_play"]:
        st.session_state["auto_play"] = auto_play_toggle
        st.rerun()
    st.sidebar.divider()
    ...
    if uploaded_file is not None:
        ...
        # =====================================================================
        st.markdown('<div class="gradient-divider" style="margin-top:0;"></div>', unsafe_allow_html=True)
        menu_col1, menu_col2, menu_col3, menu_col4 = st.columns(4)
        
        if menu_col1.button("📊 1. Executive Summary", use_container_width=True, type="primary" if st.session_state["current_slide"] == 1 else "secondary"): 
            st.session_state["current_slide"] = 1; st.rerun()
        if menu_col2.button("📈 2. Trends & Time", use_container_width=True, type="primary" if st.session_state["current_slide"] == 2 else "secondary"): 
            st.session_state["current_slide"] = 2; st.rerun()
        if menu_col3.button("🏗️ 3. Contractors", use_container_width=True, type="primary" if st.session_state["current_slide"] == 3 else "secondary"): 
            st.session_state["current_slide"] = 3; st.rerun()
        if menu_col4.button("🤖 4. AI & Quality", use_container_width=True, type="primary" if st.session_state["current_slide"] == 4 else "secondary"): 
            st.session_state["current_slide"] = 4; st.rerun()
        ...
        # =====================================================================
        # 📌 BOTTOM PRESENTATION NAVIGATION (NEXT/PREV)
        # =====================================================================
        st.markdown('<div class="gradient-divider" style="margin-bottom:0;"></div>', unsafe_allow_html=True)
        b_col1, b_col2, b_col3 = st.columns([1, 2, 1])
        with b_col1:
            if st.button("⬅️ Previous Slide", use_container_width=True):
                st.session_state["current_slide"] = 4 if st.session_state["current_slide"] == 1 else st.session_state["current_slide"] - 1
                st.rerun()
        with b_col2:
            st.markdown(f"<div style='text-align:center; color:{ui['text_muted']}; font-weight:bold; font-size:18px; padding-top:10px;'>Slide {st.session_state['current_slide']} of 4</div>", unsafe_allow_html=True)
        with b_col3:
            if st.button("Next Slide ➡️", use_container_width=True):
                st.session_state["current_slide"] = 1 if st.session_state["current_slide"] == 4 else st.session_state["current_slide"] + 1
                st.rerun()

        # =====================================================================
        # ⏱️ AUTO-PLAY ENGINE (Runs if toggled ON in Sidebar)
        # =====================================================================
        if st.session_state["auto_play"]:
            time.sleep(10) # مدة الشريحة 10 ثواني (ممكن تغيرها)
            st.session_state["current_slide"] = 1 if st.session_state["current_slide"] == 4 else st.session_state["current_slide"] + 1
            st.rerun()
